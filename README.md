...
Для того чтобы программа работала, нужно установить на компьютер Python, а также модуль Flask. Затем скачать все файлы из репозитория (с расширениями .py и .html, а также текст словаря), положив их в одну папку. При этом все HTML-файлы нужно положить во вложенную папку с названием templates. Далее нужно запустить файл login.py и перейти по ссылке в выдаче.
